...
contract ProofRegistry {
    ...
    function verifyERC20(bytes calldata proof, ERC20 token, uint256 reward)
        external
        payable
        returns (bool, PROOF_STATUS)
    {
        bytes32 proofHash = keccak256(proof);
    ...
    function verify(bytes calldata proof) external payable returns (bool, PROOF_STATUS) {
        ...
        if (isValidProof[proofHash].verifiedBy == address(0x0) && isValidProof[proofHash].verificationTimestamp == 0) {
            ...
            emit ProofVerificationClaimEvent(
                proofHash, isValid, reward, ERC20(address(0x0)), block.timestamp + CHALLENGE_PERIOD
            );
            return (isValid, PROOF_STATUS.FINALISED);
        } else {
            ProofVerificationClaim memory proofWitness = isValidProof[proofHash];
    ...
    function challenge(bytes calldata proof) external {
        ...
        claims[proofHash][originalVerifier] = RewardData({reward: 0, token: address(0x0), finalisationTimestamp: 0});
    ...
    function getProofVerificationContract(bytes calldata proof) internal pure returns (IVerifier) {
